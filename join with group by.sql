...
from students as s join enrollments as e where s.student_id=e.student_id;

select  s.student_id , s.student_name, e.enrollment_id, e.course_name 
from students as s left join enrollments as e on s.student_id=e.student_id;

-- null value nikalne ke liye is null use krte hai

select  s.student_id , s.student_name, e.enrollment_id, e.course_name 
from students as s left join enrollments as e on  s.student_id=e.student_id where enrollment_id is null;
...
select  s.student_id , s.student_name, e.enrollment_id, e.course_name 
from students as s left join enrollments as e on  s.student_id=e.student_id where enrollment_id is null;
-- group by mai couunt primary key pr lgate hai
select  s.student_id,s.student_name, count(e.enrollment_id)
from students as s join enrollments as e on  s.student_id=e.student_id group by s.student_id;

select  s.student_id,s.student_name, count(e.enrollment_id)
from students as s join enrollments as e on  s.student_id=e.student_id group by s.student_id HAVING COUNT(e.enrollment_id) > 2;

select major ,count(student_id) from students group by major;
 select s.major ,sum(e.credits) from students as s 
 join enrollments as e on s.student_id=e.student_id group by s.major;
 -- write a sql query to group students by the length of their names and show the count of students in each group 
 select char_length(student_name),count(student_id) from students group by char_length(student_name);
 select char_length(s.student_name),count(s.student_id),count(e.enrollment_id) from students as s
 join enrollments as e on s.student_id=e.student_id group by char_length(s.student_name);
 -- using limit
 select char_length(s.student_name),count(s.student_id),count(e.enrollment_id) from students as s
 join enrollments as e on s.student_id=e.student_id group by char_length(s.student_name) order by count(e.enrollment_id) desc limit 1;
